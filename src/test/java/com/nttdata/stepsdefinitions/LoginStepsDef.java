...
import com.nttdata.steps.ItemSteps;
...
public class LoginStepsDef {
    ...
    private ItemSteps itemSteps(WebDriver driver){
        return new ItemSteps(driver);
    }
...
//    }
    ...
    public void reviso_que_el_carrito_de_compras_tenga_un_item_agregado() {
        inventorySteps(driver).goToCart();
        int itemsCart = itemSteps(driver).getItemCartSize();
        //prueba: validar que al menos exista un item
        screenShot();
        Assertions.assertTrue(itemsCart > 0, "El tamaño de la lista es: " + itemsCart);
    }

    @Y("valido que el item agregado se llame {string}")
    public void reviso_que_el_carrito_de_compras_tenga_un_item_agregado(String expectedName) {
        String itemName =  itemSteps(driver).getItemName();
        //prueba: validamos el título del producto
        Assertions.assertEquals(expectedName, itemName);
    }
